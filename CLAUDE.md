...
- **최신성 유지**: 코드를 수정하면 연관된 도메인 문서를 **실시간으로 업데이트**하여 '살아있는 문서'로 관리한다.
...
### 6-1. 템플릿 준수 및 작성 절차
1. 모든 새 파일은 해당 폴더의 `_template.md` 내용을 기준으로 작성한다.
...
- **탐색 최적화**: 대규모 프로젝트라고 판단될 경우 분석 시 `grep_search`와 `glob`을 우선 사용하여 읽기 범위를 최소화한다.
- **선택적 로드**: [프로젝트 카드 + 관련 도메인 맵 + context-log.md + 최신 세션]만 선택적으로 로드하여 컨텍스트를 최적화한다.
...
- **Git 조작 제약**: 명시적 요청 없이 `git commit`, `push` 금지.
- **자격 증명 보호**: `.env`, API Key 등 민감 정보 기록 금지.
